...
public class Assignment_SplineConveyor : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        float diff = 1f / boxes.Length;

        float wayPointTimeTotal = 0f;

        for (int i = 0; i < waypoints.Length; i++)
        {
            speeds[i] = speedCurve.Evaluate((1f/waypoints.Length)*i);
            speeds[i] = 1f / speeds[i];
            wayPointTimeTotal += speeds[i];
        }
        ...
        {
            float percent = Mathf.Repeat(globalT + diff * i, 1f) * wayPointTimeTotal;

            int zoneIndex;
            for(zoneIndex =0; zoneIndex < speeds.Length; zoneIndex++)
            {
                if (percent < speeds[zoneIndex])
                {
                    break;
                }
                else
                {
                    percent-=speeds[zoneIndex];
                }
            }

            float speedAppliedT = (1f / speeds.Length) * zoneIndex + (percent / speeds[zoneIndex])/ speeds.Length;
            boxes[i].position = EvaluateSpline(waypoints, speedAppliedT);
            if (i == 0)
            {
                currentSpeedMultiplier = speedAppliedT;
            }
        }
