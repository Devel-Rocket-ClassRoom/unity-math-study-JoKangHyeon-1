...
public class Assignment_LissajousCurve : MonoBehaviour
{
    ...
    private Vector3 CalculateLissajousPosition(float time)
    {
        ...
        float x = amplitudeX * Mathf.Sin(2 * Mathf.PI * frequencyX * time + phaseXRad);
        float z = amplitudeZ * Mathf.Sin(2 * Mathf.PI * frequencyZ * time + phaseZRad);

        Vector3 newPos = new Vector3(initialPosition.x + x, initialPosition.y, initialPosition.z + z);
        return newPos;
    }
