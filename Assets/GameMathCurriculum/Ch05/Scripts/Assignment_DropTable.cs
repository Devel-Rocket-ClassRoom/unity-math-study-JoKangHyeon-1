...
public class Assignment_DropTable : MonoBehaviour
{
    ...
    private void PerformDrop()
    {
        ...
        int dropIndex;
        for (dropIndex = 0; dropIndex < items.Length; dropIndex++)
        {
            if (randomValue < items[dropIndex].weight)
            {
                break;
            }
            else
            {
                randomValue -= items[dropIndex].weight;
            }
        }

        lastSelectedIndex = dropIndex;
        dropHistory.Add(lastSelectedIndex);
        while (dropHistory.Count > MAX_HISTORY)
        {
